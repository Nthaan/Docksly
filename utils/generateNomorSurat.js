...
export async function generateNomorSurat(departemen) {
  const currentYear = new Date().getFullYear();
  const romanMonths = ["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII"];
  const month = romanMonths[new Date().getMonth()];

  const lastLetter = await prisma.surat.findFirst({
    where: {
      departemen,
      tanggal: {
  ...
  let nextNumber = "001";
  if (lastLetter?.refNumber) {
    const parts = lastLetter.refNumber.split("/");
  ...
  // ⚠️ Hanya return string, tanpa insert
  return `${nextNumber}/${departemen}/${month}/${currentYear}`;
}
